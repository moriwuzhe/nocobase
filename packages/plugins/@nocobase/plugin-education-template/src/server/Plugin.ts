...
import { createEducationRoles } from './roles';
...
export default class PluginEducationTemplateServer extends Plugin {
 ...
 try { const r = await seedData(this.db); if (r.created > 0) this.app.logger.info(`[education] Seeded ${r.created} records`); } catch (e) { this.app.logger.warn(`[education] Seed skipped: ${(e as any).message}`); }
        try { const rc = await createEducationRoles(this.app); if (rc > 0) this.app.logger.info(`[education] Created ${rc} roles`); } catch (e) { this.app.logger.warn(`[education] Roles skipped: ${(e as any).message}`); }
try { const wf = await createEducationWorkflows(this.app); if (wf > 0) this.app.logger.info(`[education] Created ${wf} workflows`); } catch (e) { this.app.logger.warn(`[education] Workflows skipped: ${(e as any).message}`); }
    try { await createTemplateUI(this.app, '教务管理', 'ReadOutlined', [{ title: '学生管理', icon: 'UserOutlined', collectionName: 'eduStudents', fields: ['studentId','name','grade','className','gender','status'], formFields: ['name','grade','className','gender','status'] }, { title: '课程管理', icon: 'BookOutlined', collectionName: 'eduCourses', fields: ['name','teacher','credits'], formFields: ['name','teacher','credits'] }]); } catch (e) { this.app.logger.warn(`[education] UI skipped: ${(e as any).message}`); }
  }
  async load() {
